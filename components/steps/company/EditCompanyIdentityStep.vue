<template>
  <section class="company_identity_form_step_wrapper">
    ...
    >
      <!-- Start:: Logo Input -->
      <div>
        <div class="logo_current">
          <div class="logo_current_founded" v-if="companyIdentityInfo.image">
            <img
...
export default {
  ...
  data() {
    return {
      ...
      imageStateText: this.$t(`OTHERS.imageIsFoundedDesc`),
  ...
  methods: {
    handleImageError(event) {
      event.target.src =
        'https://api-nemrh-test.cmt-saudi.com/assets/images/companies/company.jpg'
      this.imageStateText = this.$t(`OTHERS.imageIsNotFoundedDesc`)
    },
    ...
    submitForm() {
      if (!this.data.commercialRegister.value) {
